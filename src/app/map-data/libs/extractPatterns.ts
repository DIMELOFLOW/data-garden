...
function getNestedHeadersWithTypes(obj: DataObject, prefix: string = ''): { [key: string]: string } {
...
export function getHeadersWithTypesJson(dataArchive: string): { [key: string]: string } | null {
  if (!dataArchive) {
    return null;
  }
...
export function getHeadersWithTypesCsv(csvString: string):  string {
  try {
    ...
    const csv = headerRow
    .map((value: string, index: number) => `${headerNames.split(',')[index]}: ${typeof inferDataType(value)}`)
    .join(', ');
